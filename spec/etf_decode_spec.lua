...
describe('etf.decoder', function()
  it('is a function', function()
    assert.is_function(etf.decode)
  end)

  it('accepts a version', function()
    local dec = etf.decode('\131\97\0',{version = 131})
    assert.is.same(0,dec)
  end)

  it('errors on unknown version', function()
    assert.has_error(function()
      etf.decode('\131\97\0',{version = 0})
    end)
  end)
  ...
  describe('decodes', function()
    it('ZLIB-compressed', function()
      ...
      local res = 'hello'
      assert.are.same(res,etf.decode(bin))
    end)


    it('ATOM_CACHE_REF as nil', function()
      local bin = '\131\82\1'
      assert.is_nil(etf.decode(bin))
    end)
    ...
    it('REFERENCE_EXT', function()
      ...
      local reference = etf.decode(bin)
  ...
  it('accepts an atom_map table', function()
    local val = etf.decode('\131\115\5\104\101\108\108\111', {
      atom_map = {
        ['hello'] = 'hi',
      }
    })
  ...
  it('accepts at atom_map function', function()
    local val = etf.decode('\131\115\5\104\101\108\108\111', {atom_map = function(str)
      return str .. str
    end})
  ...
  it('allows mapping atom keys and values separately', function()
    local atom_map = function(str, is_key)
      if is_key then
        return str
      end
      return etf.atom(str)
    end
    local val = etf.decode('\131\116\0\0\0\1\100\0\1\97\100\0\1\98',{
      atom_map = atom_map,
    })
    assert.are.same({
      ['a'] = etf.atom('b')
  ...
  describe('allows returning all ints as integers', function()
    it('returns a SMALL_INTEGER_EXT as integer', function()
      local val = etf.decode('\131\97\0', { use_integer = true })
    ...
    it('returns INTEGER_EXT as integer', function()
      local val = etf.decode('\131\98\0\0\1\0', { use_integer = true })
      assert.is_userdata(val)
    ...
    it('returns SMALL_BIG_EXT as integer', function()
      local val = etf.decode('\131\110\1\0\255', { use_integer = true })
      assert.is_userdata(val)
    ...
    it('returns LARGE_BIG_EXT as integer', function()
      local val = etf.decode('\131\111\0\0\0\1\0\255', { use_integer = true })
      assert.is_userdata(val)
      assert.are.same(val,etf.integer(255))
      assert.are.same(etf.integer_mt,debug.getmetatable(val))
    end)
